    ...
    namespace Path {
        ...
        const std::string getResourcePath() {
            ...
            if(_isFolder(resPath + "./GameData"))
                ...
                throw Utils::LoadingException(
                    "Can't find the GameData directory in the game directory. "
                    "Can't run the game without data, recheck your "
                    "installation.",
                    true);
#endif
          ...
          data_dir += "/usr/local/share";
            ...
            { // search in all folders of $XDG_DATA_DIRS
                ...
                while(next_delimiter != std::string::npos) {
                    ...
                    std::string data_dir =
                        data_dir_list.substr(0, next_delimiter);
                    if(!data_dir.empty() && _isFolder(data_dir + "/OpMon")) {
                        return data_dir + "/OpMon/";
                    }
