...
namespace OpMon::Controller {
        ...
        case sf::Event::KeyPressed:
            if(menu.getCurrentOption() == View::OptionType::CONTROLS && keyChangeActive) {
                if(currentKeyChange < controlsName.size()) {
                    ...
                    if(!keyCode.empty()) {
                        Model::OptionsSave::addParam(std::string("control.") + controlsName[currentKeyChange], keyCode);
                    }
                    ++currentKeyChange;
    ...
    void OptionsMenuCtrl::raiseVolume() {
        auto newVolume = std::min(100.f, data.getUiDataPtr()->getJukebox().getGlobalVolume() + 10.f);
        data.getUiDataPtr()->getJukebox().setGlobalVolume(newVolume);
        Model::OptionsSave::modifyParam("volume", std::to_string(int(newVolume)));
    }

    void OptionsMenuCtrl::lowerVolume() {
        auto newVolume = std::max(0.f, data.getUiDataPtr()->getJukebox().getGlobalVolume() - 10.f);
        data.getUiDataPtr()->getJukebox().setGlobalVolume(newVolume);
        Model::OptionsSave::modifyParam("volume", std::to_string(int(newVolume)));
    }
