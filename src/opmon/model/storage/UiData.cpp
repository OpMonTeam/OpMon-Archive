    ...
    namespace Model {
        ...
        UiData::UiData(View::Window &win)
          : win(win) {
            ...
            listOp.emplace(31, new Species(52, 43, 60, 50, 65, 39, "Titbraise", Type::BURNING, Type::NOTHING, 0, 16, new E_Level(5, 16), std::vector<Stats>{Stats::ATK}, 0.2, 2.3, "C'est l'un des plus petit monstres de ce monde, il est assez inssignifiant mais il fait très mal quand il le veut", 65, 1059860, 45, 30));
            listOp.emplace(32, new Species(92, 87, 75, 85, 76, 90, "Bolderi", Type::TOXIC, Type::GROUND, 0, -1, new E_Nope(), std::vector<Stats>{Stats::HP, Stats::HP, Stats::HP}, 1.3, 60, "Il controle le riz, il est capable de faire le faire voler, et même de faire des formes avec.", 194, 1059860, 45, 31));
            listOp.emplace(33, new Species(57, 40, 40, 40, 50, 46, "Dragonbolderi‚", Type::TOXIC, Type::NOTHING, 0, 16, new E_Level(33, 16), std::vector<Stats>{Stats::ATK}, 0.5, 9, "Ce dragon en riz est capable d'éviter les coups en creant des trous dans son corps, mais à part cela, il peut à peine voler.", 60, 1059860, 235, 32));
            listOp.emplace(34, new Species(72, 57, 55, 55, 65, 61, "Kameosan", Type::TOXIC, Type::NOTHING, 0, -1, new E_Item(34, Item::getItem("Pierre Lune")), std::vector<Stats>{Stats::ATK, Stats::ATK}, 0.9, 19.5, "Cette jarre de saké japonais a pris vie, elle peut aspirer du riz venant de l'exterieur, pour grossir encore son crâne.", 118, 1059860, 120, 33));
