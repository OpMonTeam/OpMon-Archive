...
namespace OpMon::Model {
    ...
    Attack *Attack::newAtk(const std::string &name) {
        try {
    ...
    void Attack::initAttacks(const std::string &file) {
        ...
        for(auto &itor : json) {
            ...
            int i = 0;
            for(auto & eitor : itor.at("effects")) {
                if(!eitor.at("null")) {
            ...
            for(auto & aitor : itor.at("animations")) {
    ...
    Attack::Attack(const std::string &nameKey, int power, Type type, int accuracy, bool special, bool status, int criticalRate, bool neverFails, int ppMax, int priority, std::vector<TurnActionType> animationOrder, std::queue<View::Transformation> opAnimsAtk, std::queue<View::Transformation> opAnimsDef, std::queue<std::string> animations, AttackEffect *preEffect, AttackEffect *postEffect, AttackEffect *fails)
      ...
      , failEffect(fails)
      , animationOrder(std::move(std::move(animationOrder)))
      , opAnimsAtk(std::move(std::move(opAnimsAtk)))
      , opAnimsDef(std::move(std::move(opAnimsDef)))
      , animations(std::move(std::move(animations))) {
    }
    ...
    int Attack::attack(OpMon &atk, OpMon &def, std::queue<TurnAction> &turnQueue, bool attacker) {
        ...
        //Attack fail
        if((Utils::Misc::randU(100)) > (accuracy * (atk.getStatACC() / def.getStatEVA())) && !neverFails) {
        ...
        if(ArrayTypes::calcEffectiveness(type, def.getType1(), def.getType2()) == 0 && (!neverFails || !status)) {
        ...
        if(!status) { //Check if it isn't a status attack to calculate the hp lost
            if (!special) {
                hpLost = int(float(power) / 50.f * (atk.getLevel() * 0.4 + 2) * atk.getStatATK() / def.getStatDEF()) + 2;
            } else {
                hpLost = int(float(power) / 50.f * (atk.getLevel() * 0.4 + 2) * atk.getStatATKSPE() / def.getStatDEFSPE()) + 2;
            }
            if(type == atk.getType1() || type == atk.getType2()) {
                hpLost = int(float(hpLost) * 1.5f);
            }
            ...
            hpLost = int(float(hpLost) * effectiveness);
            if(Utils::Misc::randU(criticalRate) == 1) {
                hpLost = int(float(hpLost) * 1.5f);
            }
            hpLost = int(float(hpLost) * float(Utils::Misc::randU(100 - 85 + 1) + 85) / 100.f);
