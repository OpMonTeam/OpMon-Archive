...
namespace OpMon::Model {
    ...
    Map::Map(std::vector<int> const &layer1, std::vector<int> const &layer2, std::vector<int> const &layer3, int w, int h, bool indoor, std::string bg, std::vector<std::string> animatedElements)
      : indoor(indoor)
      , bg(std::move(bg))
      , animatedElements(std::move(animatedElements))
      , loaded(true) {
    ...
    Map::Map(nlohmann::json jsonData)
      : jsonData(std::move(std::move(jsonData)))
      , loaded(false) {
    }
    ...
    Map *Map::loadMap(OverworldData &data) {
        if(!loaded) {
            ...
            for(auto &eitor : jsonData.at("events")) {
