...
Map::Map(const int layer1[], const int layer2[], const int layer3[], int w, int h, const char *const *collision,
         sf::Music *bg, std::vector<std::vector<sf::Texture> > const &animatedElements,
         std::vector<sf::Vector2f> const &elementsPos) {
...
std::vector<Event *> Map::getEvent(sf::Vector2i position) const {
  ...
  for (Event *event : events) {
    if (event->getPosition().x CASES == position.x && event->getPosition().y
    CASES == position.y) {
