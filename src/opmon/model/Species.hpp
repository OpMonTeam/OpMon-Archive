...
namespace OpMon {
    ...
    class Species {
      private:
        ...
        Species *
            evolution; /*!< \biref The species in which the current species evolves.*/
        Evolution *evolType; /*!< \brief The way the species evolves*/
        Type type1;
        Type type2;
        std::string
            opdexEntry; /*!< \brief The descrption of the species in the OpDex.*/
        float height;
        float weight;
        ...
        std::vector<Stats>
            EVgiven; /*!< \brief The EVs given when one OpMon of this species is defeated.*/

        unsigned int
            expGiven; /*!< \brief The base EXP given when one OpMon of this species if defeated.*/
      ...
      public:
         ...
         */
        Species(unsigned int atk, unsigned int def, unsigned int atkSpe,
                unsigned int defSpe, unsigned int spe, unsigned int hp,
                std::string name, Type type1, Type type2, Evolution *evolType,
                std::vector<Stats> evGiven, float height, float weight,
                std::string opdexEntry, unsigned int expGiven, int expMax,
                int captureRate, int opdexNumber);
        unsigned int getBaseAtk() const { return baseAtk; }
        unsigned int getBaseDef() const { return baseDef; }
        unsigned int getBaseAtkSpe() const { return baseAtkSpe; }
        unsigned int getBaseDefSpe() const { return baseDefSpe; }
        unsigned int getBaseSpe() const { return baseSpe; }
        unsigned int getBaseHP() const { return baseHP; }
        std::string getName() const { return name; }
        int getCaptureRate() const { return captureRate; }
        Type getType1() const { return type1; }
        Type getType2() const { return type2; }
        unsigned int getExp() const { return expGiven; }
        int getExpMax() const { return expMax; }
        Evolution *getEvolType() const { return evolType; }
        CurveExp *getCurve() const { return curve; }
        Species *getEvolution() const { return evolution; }
        float getWeight() const { return weight; }
        float getHeight() const { return height; }
        std::vector<Stats> getEv() const { return EVgiven; }
        int getOpdexNumber() const { return this->opdexNumber; }
    };
} // namespace OpMon
#endif // ESPECE_HPP
