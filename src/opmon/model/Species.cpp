...
#pragma GCC diagnostic ignored "-Wunused-parameter"
    Species::Species(unsigned int atk, unsigned int def, unsigned int atkSpe,
                     unsigned int defSpe, unsigned int spe, unsigned int hp,
                     std::string name, Type type1, Type type2,
                     Evolution *evolType, std::vector<Stats> EVGiven,
                     float height, float weight, std::string opdexEntry,
                     unsigned int expGiven, int expMax, int captureRate,
                     int opdexNumber) {
        ...
        this->evolution =
            ((evolType != nullptr) ? this->evolType->getEvolution() : nullptr);
        this->weight = weight;
        this->height = height;
