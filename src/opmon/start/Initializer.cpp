...
namespace OpMon{
  ...
  namespace Initializer {
    ...
    void initOpMons() {
      ...
      Data::OpMons::listOp.try_emplace(10, 30, 35, 20, 20, 45, 45, "Verpomme", Type::INSECTE, Type::ROCHE, 0, 7, new E_Level(11, 19), std::vector<Stats>{Stats::HP}, 0.3, 2.9, "Les Verpommes adorent aller dans les arbres pour faire semblant d'être des pommes, pour manger des sautinelles.", 53, 1000000, 255, 10);
      Data::OpMons::listOp.try_emplace(11, 20, 55, 25, 25, 30, 50, "Terrapatte", Type::INSECTE, Type::ROCHE, 0, 10, new E_Level(12, 25), std::vector<Stats>{Stats::HP, Stats::DEF}, 0.7, 9.9, "Les Terrapattes detestent qu'on ls prennent pour des Serpiterre", 72, 1000000, 120, 11);
      Data::OpMons::listOp.try_emplace(12, 105, 40, 70, 80, 75, 55, "Millepaterre", Type::ROCHE, Type::COMBAT, 0, -1, nullptr, std::vector<Stats>{Stats::DEF, Stats::DEF, Stats::DEFSPE}, 1.1, 32, "Cet OPMon contient plus d'un millier de petits pattes collantes, ce qui lui permet de se déplacer partout", 160, 1000000, 45, 12);
      Data::OpMons::listOp.try_emplace(13, 35, 30, 20, 20, 50, 40, "Sautinelle", Type::INSECTE, Type::NOTHING, 0, 7, new E_Level(14, 7),std::vector<Stats>{Stats::SPE}, 0.3, 3.2, "Son aiguillon empoisonné est très dangereux. Son corps est coloré afin de repousser ses ennemis.", 52, 1000000, 255, 13);
      Data::OpMons::listOp.try_emplace(14, 25, 50, 25, 25, 35, 45, "Invavore", Type::INSECTE, Type::COMBAT, 0, 10, new E_Level(15, 10), std::vector<Stats>{Stats::SPE, Stats::DEF}, 0.6, 10, "Incapable de se déplacer de lui-même, il se défend en durcissant sa carapace.", 71, 1000000, 120, 14);
      Data::OpMons::listOp.try_emplace(15, 80, 40, 45, 80, 75, 65, "Assautrèle", Type::INSECTE, Type::COMBAT, 0, -1, nullptr, std::vector<Stats>{Stats::DEF, Stats::DEF, Stats::DEFSPE}, 1, 29.5, "Il se sert de ses trois aiguillons empoisonnés pour attaquer sans relâche ses adversaires.", 159, 1000000, 45, 15);
      Data::OpMons::listOp.try_emplace(16, 45, 40, 35, 35, 56, 40, "Pelicamp", Type::NORMAL, Type::VOL, 0, 18, new E_Level(17, 18), std::vector<Stats>{Stats::ATK}, 0.3, 1., "Cet OpMon adore voler seul, le problème, c'est qu'il n'est pas le seul dans le ciel...", 55, 1059860, 255, 16);
      Data::OpMons::listOp.try_emplace(17, 60, 55, 50, 50, 71, 63, "Grand-Bec", Type::NORMAL, Type::VOL, 0, 36, new E_Level(18, 36), std::vector<Stats>{Stats::ATK, Stats::SPE}, 1.1, 30, "Il protège son territoire avec ardeur et repousse à  coup de bec tout intrus.", 113, 1059860, 120, 17);
      Data::OpMons::listOp.try_emplace(18, 80, 75, 50, 40, 190, 85, "Majelanion", Type::NORMAL, Type::VOL, 0, -1, nullptr, std::vector<Stats>{Stats::ATK, Stats::SPE, Stats::SPE}, 1.5, 39.5, "Il adore attrapper des Ornitaupes en vol, sa vitesse et tellement forte que son corp siffle pednant son vol", 172, 1059860, 45, 18);
      Data::OpMons::listOp.try_emplace(19, 56, 35, 25, 35, 72, 30, "Ornitaupe", Type::NORMAL, Type::EAU, 0, 20, new E_Level(20, 27), std::vector<Stats>{Stats::ATKSPE}, 1.2, 7.5, "Cet OpMon creuse des tunnels à l'aide de sa queue plate, grâce à cela, son espece a developper dans cette queue une grande force.", 57, 1000000, 255, 19);
      Data::OpMons::listOp.try_emplace(20, 81, 60, 50, 70, 97, 55, "Ornitank", Type::NORMAL, Type::EAU, 0, -1, nullptr, std::vector<Stats>{Stats::ATKSPE, Stats::HP}, 1.5, 38.5, "Il tire des balles grâce à son ventre, cela peut transpercer une montagne.", 127, 1000000, 127, 20);
      Data::OpMons::listOp.try_emplace(21, 60, 30, 31, 31, 70, 40, "Cridaile", Type::NORMAL, Type::VOL, 0, 20, new E_Level(22, 20), std::vector<Stats>{Stats::SPE}, 0.4, 4, "Il chasse les Sautinelles dans les hautes herbes. Ses petites ailes lui permettent de voler très vite.", 58, 1000000, 255, 21);
      Data::OpMons::listOp.try_emplace(22, 160, 65, 31, 31, 10, 95, "Himondelle", Type::POISON, Type::VOL, 0, -1, nullptr, std::vector<Stats>{Stats::SPE, Stats::SPE}, 1.9, 190, "Cet OPMon plutôt imposant, ne peut plus voler depuis qu'il a évoluer, cela resulte d'un manque de migrations du à l'abondance d'insectes dans cette région", 162, 1000000, 90, 22);
      Data::OpMons::listOp.try_emplace(23, 60, 44, 40, 54, 55, 35, "Serpiterre", Type::SOL, Type::TENEBRES, 0, 24, new E_Level(24, 22), std::vector<Stats>{Stats::SPE}, 2, 6.9, "Plus il est âgé, plus son corps est long. Il se nourrit de spectres nocturnes caché sous terre", 62, 1000000, 255, 23);
      Data::OpMons::listOp.try_emplace(24, 85, 69, 65, 79, 80, 60, "Serpenair", Type::VOL, Type::TENEBRES, 0, -1, nullptr, std::vector<Stats>{Stats::SPE, Stats::ATK}, 3.5, 65, "Il adore planer au dessus des ses proies, les fantomes de nuits, pour jouer avec eux, avant de les aspirer.", 147, 1000000, 90, 24);
      ...
      Data::OpMons::listOp.try_emplace(26, 200, 55, 90, 80, 110, 60, "Cielectrik", Type::ELECTRIQUE, Type::NOTHING, 0, -1, nullptr, std::vector<Stats>{Stats::ATK, Stats::ATKSPE, Stats::ATKSPE}, 0., 30, "Il peut terrasser tout ses en libérant un décharge foudroyante plus de 550 000 V.", 122, 1000000, 65, 26);
      Data::OpMons::listOp.try_emplace(27, 60, 58, 30, 52, 33, 65, "Oursiflamme", Type::FEU, Type::NOTHING, 0, 28, new E_Level(19, 28),std::vector<Stats>{Stats::ATK}, 0.5, 20, "Un si gentil ourson qui est tout feu tout flamme.", 93, 1000000, 45, 27);
      Data::OpMons::listOp.try_emplace(28, 80, 75, 60, 59, 54, 78, "Boudabrun", Type::FEU, Type::NOTHING, 0, 29, new E_Level(37,29), std::vector<Stats>{Stats::ATK, Stats::ATK}, 1.9, 130.5, "Cet ours est colérique, un rien peut lui mettre le feu au poudre.", 122, 1000000, 60, 28);
      Data::OpMons::listOp.try_emplace(29, 110, 96, 92, 74, 72, 84, "Calcinours", Type::FEU, Type::COMBAT, 0, -1, nullptr, std::vector<Stats>{Stats::ATK, Stats::ATK, Stats::HP}, 2.6, 588, "Cet ours serait capable de mettre à feu et à sang n'importe quoi.", 172, 1250000, 172, 29);
      Data::OpMons::listOp.try_emplace(30, 62, 67, 55, 55, 26, 90, "Calame", Type::POISON, Type::EAU, 0, -1, new E_Item(31, Item::getItem("Pierre Eau")), std::vector<Stats>{Stats::HP, Stats::HP}, 0.9, 20, "Ce mignon petit OpMon asire les âmes de ses ennemis et les recrache pour les attaquer", 117, 1059860, 120, 30);
      Data::OpMons::listOp.try_emplace(31, 92, 87, 75, 85, 76, 90, "Bolderi", Type::POISON, Type::SOL, 0, -1, new E_Nope(), std::vector<Stats>{Stats::HP, Stats::HP, Stats::HP}, 1.3, 60, "Il controle le riz, il est capable de faire le faire voler, et même de faire des formes avec.", 194, 1059860, 45, 31);
      Data::OpMons::listOp.try_emplace(32, 57, 40, 40, 40, 50, 46, "Dragonbolderi‚", Type::POISON, Type::NOTHING, 0, 16, new E_Level(33, 16), std::vector<Stats>{Stats::ATK}, 0.5, 9, "Ce dragon en riz est capable d'éviter les coups en creant des trous dans son corps, mais à part cela, il peut à peine voler.", 60, 1059860, 235, 32);
      Data::OpMons::listOp.try_emplace(33, 72, 57, 55, 55, 65, 61, "Kameosan", Type::POISON, Type::NOTHING, 0, -1,new E_Item(34, Item::getItem("Pierre Lune")), std::vector<Stats>{Stats::ATK, Stats::ATK}, 0.9, 19.5, "Cette jarre de saké japonais a pris vie, elle peut aspirer du riz venant de l'exterieur, pour grossir encore son crâne.", 118, 1059860, 120, 33);
