...
namespace OpMon::View {
    ...
    Battle::Battle(const Model::OpTeam *atkTeam, const Model::OpTeam *defTeam, const std::string& trainerClass, const std::string& background, Model::BattleData &data)
      : atkTeam(atkTeam)
      , defTeam(defTeam)
      , data(data) {
        ...
        for(auto &i : choicesTxt) {
            i.setFont(data.getUiDataPtr()->getFont());
            i.setCharacterSize(20);

            i.setSfmlColor(sf::Color::White);
        }
