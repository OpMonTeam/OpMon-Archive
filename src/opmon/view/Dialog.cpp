...
namespace OpMon::View {
    ...
    Dialog::Dialog(const sf::String &text, Model::UiData *uidata)
      : uidata(uidata) {
    ...
    void Dialog::init() {
        ...
        int minusPos = 32;
        for(auto &i : dialogText) {
            i.setFont(uidata->getFont());
            i.setCharacterSize(FONT_SIZE_DEFAULT);
            i.setSfmlColor(sf::Color::Black);

            i.setPosition(25, background.getPosition().y + float(minusPos));
            minusPos += 32;
        }
    }
