...
namespace OpMon::Elements {
    ...
    DoorEvent::DoorEvent(OverworldData &data, nlohmann::json jsonData)
        : LinearMetaEvent(
              std::queue<AbstractEvent *>(std::deque<AbstractEvent *>(
                  {new AnimationEvent(
                       data, jsonData,
                       {sf::IntRect(0, 0, 42, 36), sf::IntRect(42, 0, 42, 36),
                        sf::IntRect(84, 0, 42, 36),
                        sf::IntRect(126, 0, 42, 36)}),
                   new SoundEvent(data, jsonData), new TPEvent(data, jsonData),
                   nullptr})),
              std::queue<bool>(std::deque<bool>({false, true, false, false}))) {
        this->sprite->setOrigin(5, 6); // The doors are a bit bigger than a
                                       // square (42x36 instead of 32x32).
    }
    ...
    TalkingCharaEvent::TalkingCharaEvent(OverworldData &data,
                                         nlohmann::json jsonData)
        : LinearMetaEvent(
              std::queue<AbstractEvent *>(std::deque<AbstractEvent *>(
                  {new CharacterEvent(data, jsonData),
                   new DialogEvent(data, jsonData), nullptr})),
    ...
    TrainerEvent::TrainerEvent(TalkingCharaEvent *prebattlenpc,
                               BattleEvent *battle,
                               TalkingCharaEvent *postbattlenpc)
        : AbstractMetaEvent(
              std::queue<AbstractEvent *>(std::deque<AbstractEvent *>(
    ...
    TrainerEvent::TrainerEvent(OverworldData &data, nlohmann::json jsonData)
        ...
        // If "postbattle" field doesn't exist, the post battle character is the
        // same as the pre battle one.
        eventQueue.push(
            jsonData.contains("postbattle") ?
                new TalkingCharaEvent(data, jsonData.at("postbattle")) :
                eventQueue.front());
    }

    void TrainerEvent::update(Overworld &overworld) {
        eventQueue.front()->update(
            overworld); // Updates the first event in the queue.
        if(triggered && !defeated &&
           eventQueue.front()
            ...
            defeated = true;
            triggered = false;
            if(eventQueue.front()->getPositionMap().getPosition() ==
               sf::Vector2i(0, 0)) // If the position of the new event is 0,0
                eventQueue.front()->setPosition(
                    garbage.front()
                        ->getPositionMap()
                        .getPosition()); // sets it to the position of the pre
                                         // battle event
            mainEvent = eventQueue.front();
        }
    ...
    void TrainerEvent::action(Overworld &overworld) {
        eventQueue.front()->action(
            overworld); // Triggers the first event in the queue.
        triggered = true;
    }
