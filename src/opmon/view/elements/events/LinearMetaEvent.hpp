...
namespace OpMon::Elements {
     ...
     * \ingroup Events
     *
     * This class takes two queues of the same size, one of events and the other
     * of booleans. If the boolean is `true`, the event will be called at the
     * same time as the previous events. Else, the meta event will wait for the
     * previous events to end before activating the new one.
     *
     * Since the processing of the queue will loop after processing have been
     * set to `true`, you can use `nullptr` to pause the loop. It can be used
     * anywhere in the queue to pause it,
    ...
    class LinearMetaEvent : public AbstractMetaEvent {
      protected:
         ...
         *
         * If the boolean is `true`, the event will be called at the same time
         * as the previous events. Else, the meta event will wait for the
         * previous events to end before activating the new one.
         */
        std::queue<bool> actionQueue;

      public:
        LinearMetaEvent(std::queue<AbstractEvent *> eventQueue,
                        std::queue<bool> actionQueue);
