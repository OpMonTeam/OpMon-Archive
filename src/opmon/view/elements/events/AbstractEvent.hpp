...
namespace OpMon {
    ...
    namespace Elements {
        ...
        enum class EventTrigger {
            PRESS = 0, /*!< The event is triggered when the player presses the
                          action key.*/
            GO_IN = 1, /*!< The event is triggered when the player is moving
                          towards the tile.*/
            ZONE =
                2, /*!< The event is triggered as soon as the player sees it.*/
            BE_IN = 3 /*!< The event is triggered as long as the player is on
                         its position.*/
        };
         ...
         *
         * The way the event is triggered can be defined with the enumeration
         * EventTrigger. The events directly inherited from AbstractEvent are
         * simple events : they must not depend on any other event. To create
         * events that use other events, see AbstractMetaEvent.
         */
        class AbstractEvent {
            friend class TalkingCharaEvent; // Needed to update currentTexture
                                            // without creating a public setter.
          protected:
            /*!
             ...
             *
             * The texture can be a table of different frames for an animation,
             * and this vector lists the bounds of each frame on the texture.
             */
          ...
          public:
             ...
             */
            AbstractEvent(sf::Texture &texture,
                          std::vector<sf::IntRect> rectangles,
                          EventTrigger eventTrigger,
            ...
            int getSide() const { return sides; }
            virtual const sf::IntRect &getFrameRect() { return *currentFrame; }
            EventTrigger getEventTrigger() const { return eventTrigger; }

            sf::Vector2f getPosition() const { return position; }

            Position getPositionMap() const { return mapPos; }

            Position &getPositionMapRef() { return mapPos; }
