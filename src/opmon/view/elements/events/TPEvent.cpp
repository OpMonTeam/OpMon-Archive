...
namespace OpMon {
    namespace Elements {
        ...
        TPEvent::TPEvent(sf::Texture &texture,
                         std::vector<sf::IntRect> rectangles,
                         EventTrigger eventTrigger,
                         sf::Vector2f const &position,
                         sf::Vector2i const &tpPos, std::string const &map,
                         Side ppDir, int sides, bool passable)
        ...
        TPEvent::TPEvent(OverworldData &data, nlohmann::json jsonData)
            : AbstractEvent(data, jsonData),
              tpCoord(sf::Vector2f(jsonData.at("tp").at("position")[0],
                                   jsonData.at("tp").at("position")[1])),
        ...
        void TPEvent::update(Overworld &overworld) {
            if(command &&
               !overworld.getCharacter().getPositionMap().isMoving()) {
