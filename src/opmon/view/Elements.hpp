...
namespace OpMon::View {
    ...
    class Transformation {
      protected:
        ...
        static double calcFormula(std::vector<double> formula, FormulaMode mode, double t);
        //Rotates a vector. The angle is in degrees.
        static sf::Vector2f rotateVector(sf::Vector2f const &vect, double angle);

        static std::vector<double> inverseFormula(std::vector<double> formula, FormulaMode mode);

      public:
        explicit Transformation(unsigned int const &time, MovementData md = MovementData(), RotationData rd = RotationData(), ScaleData sd = ScaleData(), sf::Transform *sprite = nullptr);
        ...
        static MovementData newMovementData(FormulaMode modeX, FormulaMode modeY, std::vector<double> xformula, std::vector<double> yformula, bool relative = true);
        static RotationData newRotationData(FormulaMode formulaMode, std::vector<double> formula, sf::Vector2f origin);
        static ScaleData newScaleData(FormulaMode modeX, FormulaMode modeY, std::vector<double> xformula, std::vector<double> yformula, sf::Vector2f origin);
    };
