...
namespace OpMon {
    ...
    GameStatus IntroSceneCtrl::checkEvent(sf::Event const &event) {
        ...
        switch(event.type) {
            case sf::Event::KeyPressed:
                if(event.key.code == data.getGameDataPtr()->getKeyTalk() &&
                   introscene.getDialog() != nullptr) {
                    introscene.getDialog()->pass();
                }
                // Part 1 is the part where the player enters his/her name
                if(event.key.code == sf::Keyboard::Return &&
                   introscene.getPart() == 1) {
                    sf::String &pName = introscene.getpName();
                    if(pName.isEmpty()) {
                        pName = "Player"; // We will find a default name some
                                          // day
                    }
                ...
                // P is used to skip the introduction, but it must be disabled
            ...
            case sf::Event::TextEntered:
                if(introscene.getPart() == 1) {
                    ...
                    if(event.text.unicode == 8) { // Backspace
                        if(!pName.isEmpty()) {
                            pName = sf::String::fromUtf32(pName.begin(),
                                                          pName.end() - 1);
                        }
                    } else if(event.text.unicode > 32) {
                        if(pName.toUtf32().size() < 14) { // 14 = Max name
                                                          // length
    ...
    void IntroSceneCtrl::loadNextScreen() {
        switch(loadNext) {
            case LOAD_OVERWORLD:
                _next_gs = std::make_unique<OverworldCtrl>(
                    data.getPlayer(), data.getGameDataPtr());
                break;
            case LOAD_ANIMATION_OPEN:
                _next_gs = std::make_unique<AnimationCtrl>(
                    std::make_unique<Animations::WinAnim>(screenTexture, true));
                break;
            case LOAD_ANIMATION_CLOSE:
                _next_gs = std::make_unique<AnimationCtrl>(
                    std::make_unique<Animations::WinAnim>(screenTexture,
                                                          false));
                break;
            default:
                throw Utils::UnexpectedValueException(
                    std::to_string(loadNext),
                    "a view to load in IntroSceneCtrl::loadNextScreen()");
        }
    }

    GameStatus IntroSceneCtrl::update(sf::RenderTexture &frame) {
        // If the player have finished to enter his/her name, animNext must have
        // been set to true. This part will launch the animation.
        if(animNext) {
            animNext = false;
            frame.draw(view);
        ...
        if(toReturn == GameStatus::NEXT_NLS || toReturn == GameStatus::NEXT) {
            switch(view.getPart()) {
                ...
                default:
                    throw Utils::UnexpectedValueException(
                        std::to_string(view.getPart()),
                        "1 or 3 in IntroScene::draw()");
            }
        }
    ...
    void IntroSceneCtrl::suspend() { view.pause(); }

    void IntroSceneCtrl::resume() { view.play(); }
