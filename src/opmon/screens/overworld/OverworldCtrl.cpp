...
namespace OpMon {
    ...
    GameStatus OverworldCtrl::checkEvent(sf::Event const &events) {
        ...
        bool is_dialog_open =
            overworld.getDialog() && !overworld.getDialog()->isDialogOver();
            ...
            case sf::Event::KeyPressed:
                ...
                if(view.debugMode) {
                    ...
                    if(events.key.code == sf::Keyboard::PageUp) {
                        ...
                        overworld.tp(data.getCurrentItorMap(),
                                     sf::Vector2i(5, 5));
                    }
        ...
        return is_dialog_open ? checkEventsDialog(events, overworld) :
                                checkEventsNoDialog(events);
    }

    GameStatus OverworldCtrl::checkEventsDialog(sf::Event const &events,
                                                Overworld &overworld) {
        switch(events.type) {
            case sf::Event::KeyPressed:
    ...
    GameStatus OverworldCtrl::update(sf::RenderTexture &frame) {
        bool is_dialog_open =
            view.getDialog() && !view.getDialog()->isDialogOver();
        if(!is_dialog_open) {
        ...
        switch(loadNext) {
            ...
            case LOAD_MENU_OPEN:
                _next_gs = std::make_unique<AnimationCtrl>(
                    std::make_unique<Animations::WooshAnim>(
                        screenTexture, data.getGameMenuData().getMenuTexture(),
                        Animations::WooshDir::UP, 15, true));
                break;
            case LOAD_MENU:
                _next_gs = std::make_unique<GameMenuCtrl>(
                    data.getGameMenuData(), player);
                break;
            case LOAD_MENU_CLOSE:
                _next_gs = std::make_unique<AnimationCtrl>(
                    std::make_unique<Animations::WooshAnim>(
                        screenTexture, data.getGameMenuData().getMenuTexture(),
                        Animations::WooshDir::DOWN, 15, false));
                break;
            default:
                throw Utils::UnexpectedValueException(
                    std::to_string(loadNext),
                    "a view to load in OverworldCtrl::loadNextScreen()");
        }
    }
    ...
    void OverworldCtrl::checkMove(Overworld &overworld) {
        if(!overworld.justTp &&
           !overworld.getCharacter().getPositionMap().isAnim() &&
           !overworld.getCharacter().getPositionMap().isLocked()) {
            // TODO Factorise code
            if(sf::Keyboard::isKeyPressed(
                   overworld.getData().getGameDataPtr()->getKeyUp())) {
                overworld.startPlayerAnimation();
                move(Side::TO_UP, overworld);
            } else if(sf::Keyboard::isKeyPressed(
                          overworld.getData().getGameDataPtr()->getKeyDown())) {
                overworld.startPlayerAnimation();
                move(Side::TO_DOWN, overworld);
            } else if(sf::Keyboard::isKeyPressed(
                          overworld.getData().getGameDataPtr()->getKeyLeft())) {
                overworld.startPlayerAnimation();
                move(Side::TO_LEFT, overworld);
            } else if(sf::Keyboard::isKeyPressed(overworld.getData()
                                                     .getGameDataPtr()
                                                     ->getKeyRight())) {
                overworld.startPlayerAnimation();
    ...
    void OverworldCtrl::checkAction(sf::Event const &event,
                                    Overworld &overworld) {
        // If the player isn't moving, then this checks if the player want to
        // activate an event.
        if(!overworld.getCharacter().getPositionMap().isAnim()) {
            // Get the event coordinates and activate it if the player
            // interacted with it.
            if(sf::Keyboard::isKeyPressed(
                   overworld.getData().getGameDataPtr()->getKeyInteract())) {
                int lx =
                    overworld.getCharacter().getPositionMap().getPosition().x;
                int ly =
                    overworld.getCharacter().getPositionMap().getPosition().y;
    ...
    void OverworldCtrl::actionEvents(
        ...
        for(std::list<Elements::AbstractEvent *>::iterator itor =
            ...
            if((*itor)->getEventTrigger() == toTrigger) {
                bool go = false;
                if((((*itor)->getSide() & SIDE_UP) == SIDE_UP) &&
                   ppDir == Side::TO_UP) {
                    go = true;
                } else if((((*itor)->getSide() & SIDE_DOWN) == SIDE_DOWN) &&
                          ppDir == Side::TO_DOWN) {
                    go = true;
                } else if((((*itor)->getSide() & SIDE_RIGHT) == SIDE_RIGHT) &&
                          ppDir == Side::TO_RIGHT) {
                    go = true;
                } else if((((*itor)->getSide() & SIDE_LEFT) == SIDE_LEFT) &&
                          ppDir == Side::TO_LEFT) {
                    go = true;
                }
                if(go) {
