...
namespace OpMon {
    ...
    SaveMenu::SaveMenu(SaveMenuData &data, SaveMenuContext context)
        : data(data) {
        float saveMenuItemX =
            (data.getGameDataPtr()->getWindowWidth() - SAVE_MENU_ITEM_WIDTH) /
            2;

        sf::Vector2f hintTextBoxPosition(saveMenuItemX, SAVE_MENU_ITEM_PADDING);
        hintTextBox = TextBox(data.getGameDataPtr()->getMenuFrame(),
                              hintTextBoxPosition, SAVE_MENU_ITEM_WIDTH, 64);
        ...
        for(int i = 0; i < 3; i++) {
            sf::Vector2f position(saveMenuItemX,
                                  64 + (2 * SAVE_MENU_ITEM_PADDING) +
                                      i * (96 + SAVE_MENU_ITEM_PADDING));
            TextBox saveFileTextBox(data.getGameDataPtr()->getMenuFrame(),
                                    position, SAVE_MENU_ITEM_WIDTH, 96);
        ...
        sf::Vector2f confirmationTextBoxPosition(
            saveMenuItemX, 64 + (2 * SAVE_MENU_ITEM_PADDING) +
                               3 * (96 + SAVE_MENU_ITEM_PADDING));
        float confirmationTextBoxHeight = 132;
        float confirmationTextBoxWidth =
            SAVE_MENU_ITEM_WIDTH - 128 - SAVE_MENU_ITEM_PADDING;
        confirmationTextBox = TextBox(
            data.getGameDataPtr()->getMenuFrame(), confirmationTextBoxPosition,
            confirmationTextBoxWidth, confirmationTextBoxHeight);
        confirmationTextBox.setFont(data.getGameDataPtr()->getFont());
        confirmationTextBox.setLeftContent("Are you sure?");

        float yesTextBoxX =
            saveMenuItemX + confirmationTextBoxWidth + SAVE_MENU_ITEM_PADDING;
        float yesTextBoxY = 64 + (2 * SAVE_MENU_ITEM_PADDING) +
                            3 * (96 + SAVE_MENU_ITEM_PADDING);
        sf::Vector2f yesTextBoxPosition(yesTextBoxX, yesTextBoxY);
        yesTextBox = TextBox(data.getGameDataPtr()->getMenuFrame(),
                             yesTextBoxPosition, 128, 64);
        yesTextBox.setFont(data.getGameDataPtr()->getFont());
        yesTextBox.setLeftContent("Yes");

        float noTextBoxX =
            saveMenuItemX + confirmationTextBoxWidth + SAVE_MENU_ITEM_PADDING;
        float noTextBoxY = 64 + (3 * SAVE_MENU_ITEM_PADDING) +
                           3 * (96 + SAVE_MENU_ITEM_PADDING) + 64;
        sf::Vector2f noTextBoxPosition(noTextBoxX, noTextBoxY);
        noTextBox = TextBox(data.getGameDataPtr()->getMenuFrame(),
                            noTextBoxPosition, 128, 64);
    ...
    void SaveMenu::draw(sf::RenderTarget &frame,
    ...
    GameStatus SaveMenu::fileConfirmationLoop() {
        noTextBox.setActive(currentCursorConfirmationOption ==
                            SaveMenuConfirmationOption::NO);
        yesTextBox.setActive(currentCursorConfirmationOption ==
                             SaveMenuConfirmationOption::YES);
    ...
    SaveMenuState SaveMenu::getCurrentState() { return currentState; }
