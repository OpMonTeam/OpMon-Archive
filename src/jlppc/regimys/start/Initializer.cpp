...
void initOpMons() {
    ...
    listeOp[4] = new Espece(52, 43, 60, 50, 65, 39, "Titbraise", Type::FEU, Type::AUCUN, 0, 16, new E_Level(5, 16), evs[4], 0.2, 2.3, "C'est l'un des plus petit monstres de ce monde, il est assez inssignifiant mais il fait très mal quand il le veux", 65, 1059860, 45, 4);
    listeOp[5] = new Espece(64, 58, 80, 65, 80, 58, "Rouxiflamme", Type::FEU, Type::AUCUN, 0, 36, new E_Level(6, 36), evs[5], 1.1, 19, "Quand cette créature attaque, elle rougit de toute ses forces pour bruler son adversaire", 142, 1059860, 45, 5);
    ...
    listeOp[7] = new Espece(25, 35, 50, 54, 73, 44, "Nanolphin", Type::EAU, Type::AUCUN, 0, 16, new E_Level(8, 16), evs[7], 0.5, 9, "Il adore nager en compagnie d'autres Nanolphins", 66, 1059860, 45, 7);
    listeOp[8] = new Espece(63, 80, 65, 80, 110, 59, "Deaulfin", Type::EAU, Type::AUCUN, 0, 36, new E_Level(9, 36), evs[8], 1, 22.5,"Il peut cracher de l'eau grâce à sa queue, le jet est tellement puissant qu'il peut se déplacer sur terre", 143, 1059860, 45, 8);
    listeOp[9] = new Espece(120, 80, 75, 105, 30, 109, "Roileine", Type::EAU, Type::AUCUN, 0, -1,NULL, evs[9], 1.6, 85.5, "Il est le roi des océans de la région, il est capable de manger n'importe quoi, n'importe ou.", 210, 1059860, 45, 9);
    ...
    listeOp[11] = new Espece(20, 55, 25, 25, 30, 50, "Terrapatte", Type::INSECTE, Type::AUCUN, 0, 10, new E_Level(12, 10), evs[11], 0.7, 9.9, "Son corps frêle est protégé par sa carapace d'acier. Il encaisse les coups durs en attendant d'évoluer.", 72, 1000000, 120, 11);
    ...
    listeOp[25] = new Espece(55, 40, 50, 50, 90, 35, "Nucléage", Type::ELECTRIQUE, Type::AUCUN, 0, -1, new E_Item(26, Item::getItem("Invocation de la foudre")), evs[25], 7.4, 6, "Ce nuage s'amuse à viser les monuments ou objets très grands, ils sont à l'origine des éclairs aleatoires dans le ciel. Ils aiment se caché parmi les vraies nuages", 82, 1000000, 100, 25);
    listeOp[26] = new Espece(200, 55, 90, 80, 110, 60, "Cielectrik", Type::ELECTRIQUE, Type::AUCUN, 0, -1, NULL, evs[26], 0., 30, "Il peut terrasser tout ses en libérant un décharge foudroyante plus de 550 000 V.", 122, 1000000, 65, 26);
    listeOp[27] = new Espece(60, 58, 30, 52, 33, 65, "Oursiflamme", Type::FEU, Type::AUCUN, 0, 28, new E_Level(19, 28),evs[27], 0.5, 20, "Un si gentil ourson qui est tout feu tout flamme.", 93, 1000000, 45, 27);
    listeOp[28] = new Espece(80, 75, 60, 59, 54, 78, "Oursibraise", Type::FEU, Type::AUCUN, 0, 29, new E_Level(37,29), evs[28], 1.1, 29.5, "Cet ours est colérique, un rien peut lui mettre le feu au poudre.", 122, 1000000, 60, 28);
...
void initPlans() {
    ...
    PLANS_RESET
    #ifdef _WIN32
    couche1->loadFromFile(RESSOURCES_PATH + "maps\\pproom\\pproom1.png");
    couche2->loadFromFile(RESSOURCES_PATH + "maps\\pproom\\pproom2.png");
    couche3->loadFromFile(RESSOURCES_PATH + "maps\\pproom\\pproom3.png");
    #else
    couche1->loadFromFile(RESSOURCES_PATH + "maps/pproom/pproom1.png");
    couche2->loadFromFile(RESSOURCES_PATH + "maps/pproom/pproom2.png");
    couche3->loadFromFile(RESSOURCES_PATH + "maps/pproom/pproom3.png");
    #endif // _WIN32
    plans.push_back(new Plan(*couche1, *couche2, *couche3, 9, 6, "colPProom", townMusics[0]));
    PLANS_RESET
