...
namespace Utils::Origin {
     ...
     *
     * It has to be a template function, because we need setOrigin() from
     * Transformable and getLocalBounds() from the Drawable implementations.
    ...
    void centerOrigin(T &transformable) {
        ...
        transformable.setOrigin(std::floor(bounds.left + bounds.width / 2.f),
